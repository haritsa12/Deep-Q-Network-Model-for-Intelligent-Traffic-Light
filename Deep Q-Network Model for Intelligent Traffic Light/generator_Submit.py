...
class TrafficGenerator:
    def __init__(self, max_steps, total_cars):
        self._total_cars = total_cars  # how many cars per episode
        self._max_steps = max_steps

    def route_generator(self, seed):
        """
        ...
        timings = np.random.weibull(2, self._total_cars)
        timings = np.sort(timings)

        # Mapping a numeric range onto another range
        car_gen_steps = []
        ...
        with open("intersection/episode_routes.rou.xml", "w") as routes:
            print("""<routes>
            <vType accel="1.0" decel="4.5" id="standard_car" length="5.0" minGap="2.5" maxSpeed="70" sigma="0.5" />
